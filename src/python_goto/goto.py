...
class _Bytecode:
    def __init__(self: _Bytecode) -> None:
        """If linter says x and y are not defined don't remove them this is something
        that linter can't understand unless there is a advanced modification on your
        linter.

        Starting with Python 3.6, the bytecode format has changed, using 16-bit words 
        (8-bit opcode + 8-bit argument) for each instruction, as opposed to previously 
        24-bit (b-bit opcode + 16-bit argument) for instructions that expect and 
        argument and otherwise 8-bit.

        https://bugs.python.org/issue26647

        If you have noticed then we are also changing the jump target here, why? As of 
        Python 3.6, jump targets are still addressed by their byte unit. This is a matter 
        to change, so that jump targets, in the future might refer to code units (address 
        in bytes / 2).
    ...
    @property
    def argument_bits(self: _Bytecode) -> int:
...
def _make_code(code: function, codestring: bytearray) -> object:
    args = [
...
def _parse_instructions(code: function) -> tuple:
    pos = 0
    extended_arg = 0
...
def _get_instructions_size(ops: str) -> int:
    size = 0
...
def _write_instruction(buf: list, pos: int, opname: str, oparg: int = 0) -> int:
...
def _write_instructions(buf: list, pos: int, ops: str) -> int:
    for op in ops:
        if isinstance(op, str):
...
def _find_labels_and_gotos(code: function) -> list:
    gotos = []
    labels = {}
        ...
        if opname1 in ("LOAD_GLOBAL", "LOAD_NAME"):
            if opname2 == "LOAD_ATTR" and opname3 == "POP_TOP":
                ...
                if name == "label":
                    ...
                    labels[oparg2] = (offset1, offset4, tuple(block_stack))
                elif name == 'goto':
                    gotos.append(
                        (offset1, offset4, oparg2, tuple(block_stack)))
        elif opname1 in ("SETUP_LOOP", "SETUP_EXCEPT", "SETUP_FINALLY", "SETUP_WITH", "SETUP_ASYNC_WITH"):
            block_counter += 1
...
def _inject_nop_sled(buf: list, pos: int, end: int) -> None:
    while pos < end:
        pos = _write_instruction(buf, pos, "NOP")


def _patch_code(code: function) -> object:
    ...
    for pos, end, label, origin_stack in gotos:
        ...
        for _ in range(len(origin_stack) - target_depth):
...
def with_goto(func_or_code: function) -> object:
